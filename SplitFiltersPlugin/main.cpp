	...
	for (int i = 0; i < sel_num; i++) {
		...
		edit->delete_object(obj);
		{
			...
			if (!new_obj0) {
				...
				MessageBox(get_aviutl2_window(), L"元オブジェクトの再作成に失敗しました。", PLUGIN_NAME, MB_ICONWARNING);
				continue;
			}
		}
		...
		if (!created) {
			...
			MessageBox(get_aviutl2_window(), L"フィルタ効果オブジェクトの作成に失敗しました。", PLUGIN_NAME, MB_ICONWARNING);
		}
...
static void __cdecl split_filters_for_group_callback(EDIT_SECTION* edit) {
	...
	for (int i = 0; i < sel_num; i++) {
		...
		if (!group_obj) {
			...
			MessageBox(get_aviutl2_window(), L"グループ制御オブジェクトの作成に失敗しました。", PLUGIN_NAME, MB_ICONWARNING);
			continue;
		}
